	...
	for (i=0; i<self._numberOfPages; i++)
	{
        ...
        else if (self.pageControlStyle==PageControlStyleThumb)
        {
            if (self.thumbImage && self.selectedThumbImage)
            {
                if (i==_currentPage)
                {
                    [self.selectedThumbImage drawInRect:CGRectMake(x,(self.frame.size.height-self.selectedThumbImage.size.width)/2,self.selectedThumbImage.size.width,self.selectedThumbImage.size.height)];
                }
                else
                {
                    [self.thumbImage drawInRect:CGRectMake(x,(self.frame.size.height-_diameter)/2,self.thumbImage.size.width,self.thumbImage.size.height)];
                }
            }
        }
	}
